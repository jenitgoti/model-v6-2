"""Model_v6.2: scratch instance segmentation with exact Model_v5 augmentation.
...
* The 95-mode Model_v5 augmentation cycle is retained exactly: one original,
  45 exact rotations from 2 to 90 degrees, and 49 realistic variants.
...
* Original and augmented centre/offset targets use one target generator.
...
* Separate best-semantic and best-instance-F1 checkpoints are saved.
...
already exist. Inspect the preview. Use ``RUN_MODE = "transfer"`` for the
...
def rasterize_instances(
    ...
    return semantic, instance, next_instance_id - 1
...
def prepare_split(split: str) -> dict[str, object]:
    ...
    for index, (image_path, label_path) in enumerate(records):
        ...
        semantic, instance, object_count = rasterize_instances(
            label_path, metadata
        )
...
def validate_augmentation_configuration() -> None:
    ...
    if EXACT_ROTATION_MODE_COUNT != 45:
        raise ValueError(
            "Expected exactly 45 rotations: 2, 4, ..., 90 degrees."
        )
...
def sample_rotation_angle(rng: np.random.Generator) -> float:
    """Sample 2, 4, ..., 90 degrees, optionally in either direction."""
    choices = np.arange(
        MIN_ROTATION_DEGREES,
        MAX_ROTATION_DEGREES + 1,
        ROTATION_STEP_DEGREES,
...
def zoom_to_fill_affine_matrix(
    ...
    """Return the unchanged Model_v5 zoom-to-fill affine matrix."""
...
def augment_training_sample(
    ...
    """Apply one unchanged Model_v5 paired augmentation mode."""
...
def semantic_class_pixel_counts(semantic_labels: np.ndarray) -> np.ndarray:
    ...
    for start in range(0, len(semantic_labels), 32):
        ...
        batch_counts = np.bincount(chunk.ravel(), minlength=NUM_CLASSES)
        if len(batch_counts) != NUM_CLASSES:
            raise ValueError(
                "Semantic labels contain a class outside 0..4. "
...
def build_model_v6_2_instance(
    ...
    semantic_p2 = C3k2(
        layers.Concatenate(name="semantic_p2_concat")(
    ...
    semantic_p1 = C3k2(
        layers.Concatenate(name="semantic_p1_concat")(
    ...
    semantic_full = C3k2(
        layers.Concatenate(name="semantic_full_concat")(
    ...
    instance_p2 = C3k2(
        layers.Concatenate(name="instance_p2_concat")(
    ...
    instance_p1 = C3k2(
        layers.Concatenate(name="instance_p1_concat")(
...
def decode_instances(
    ...
    # Convert the configured full-resolution NMS radius to the native
    # centre-head resolution.
    average_head_scale = 0.5 * (
        head_width / float(width)
        + head_height / float(height)
    )

    native_nms_radius = max(
        1,
        int(
            round(
                float(CENTER_NMS_RADIUS)
                * average_head_scale
            )
        ),
    )
...
class InstanceF1Checkpoint(tf.keras.callbacks.Callback):
    ...
    def _load_existing_history(self) -> None:
        ...
        if (
            ...
            self.evaluations_without_improvement = sum(
                ...
                and int(record["epoch"]) >= self.early_stopping_start_epoch
            )
    ...
    def on_epoch_end(
        ...
        if improved:
            print(
                "Saved new best validation-instance-F1 "
                f"checkpoint: {self.output_path} "
        ...
        if stop_requested:
            ...
            print(
                "Stopping fine-tuning: validation instance F1 did not improve "
                f"for {self.evaluations_without_improvement} scheduled "
                "evaluations."
            )
        ...
        results.append(
            {
                ...
                # Backwards-compatible confidence field.
...
def save_training_configuration(
    ...
    *,
    ...
    configuration = {
        ...
        "augmentation": {
            "source": "unchanged Model_v5 augmentation implementation",
